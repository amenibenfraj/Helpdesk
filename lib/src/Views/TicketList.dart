...
import 'package:helpdesk/src/Views/FormAddTicket.dart';
...
import 'Widget/AssignTicketDialog.dart';
...
class Ticketlist extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
       key: _scaffoldKey,
      drawer: const MenuWidget(currentIndex: 2),
      body: Container(
  ...
  return Column(
    ...
    children: [
        ...
        children: [
          ...
          SizedBox(width: 10),
            Container(
              ...
              child: Obx(() => Text(
                "${controller.filtredTicket.length} tickets",
                style: TextStyle(
      ...
      Wrap(
        ...
        runSpacing: 8.0,
        children: [
          _buildStatusChip(context, "All", Colors.blue),
          ...
          _buildStatusChip(context, "Deleted", Colors.red),
  ...
  Widget _buildStatusChip(BuildContext context, String status, Color color) {
    return Obx(() {
      ...
      return GestureDetector(
        onTap: () {
          controller.filterByStatus(status == "All" ? "" : status);
        },
        child: Container(
  ...
  Widget ticketlist() {
    return Obx(() {
      if (controller.isLoading.value && controller.filtredTicket.isEmpty) {
      ...
      if (controller.filtredTicket.isEmpty) {
        return Center(
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
        ...
        child: Scrollbar(
          ...
          child: AnimationLimiter(
            child: ListView.builder(
              ...
              itemCount: controller.filtredTicket.length,
              itemBuilder: (context, index) {
                final ticket = controller.filtredTicket[index];
                ...
                if (ticket.status == 'Not Assigned' ||
                  ...
                  showDialog(
                    ...
                    builder: (context) => AssignTicketDialog(
                      ticketId: ticket.id,
                      technicians: techController.technicians,
                    ),
                    barrierDismissible: false,
                  );
                } else {
                  Get.snackbar(
                    "Échec",
                    "Cannot assign technician to this ticket. Status: ${ticket.status}",
...
Get.toNamed('/editTicket', arguments: ticket);
          ...
          if (ticket.status != "Deleted")
            _buildActionButton(
              ...
              onPressed: () {
                if (ticket.status != "Deleted") {
                  controller.confirmDeleteTicket(context, ticket.id);
                }
              },
            ),
          if (ticket.status != "Not Assigned" && ticket.status != "Assigned")
            _buildActionButton(
              icon: Icons.chat,
              label: "Chat",
              iconColor: Color(0xFF4A6FE5),
